...
`act --validate` (and `--dryrun`) are parse-only — no container and no Docker daemon needed. Actually executing a workflow job locally does need one; with colima: `DOCKER_HOST=$(docker context inspect colima --format '{{ .Endpoints.docker.Host }}') act …` (act ignores `docker context`; see <https://nektosact.com/missing_functionality/docker_context.html>).
