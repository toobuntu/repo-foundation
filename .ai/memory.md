...
- **Dependabot vs the guard:** consumers' generated `dependabot.yml` includes the github-actions ecosystem, so Dependabot will propose SHA bumps to canonical workflow files in consumers; the foundation guard fails those PRs by design (only sync-App PRs are exempt). The org flow is: close the consumer PR, bump the pin in repo-foundation, let the sync deliver it. If the noise grows, teach the consumers' dependabot config to ignore canonical workflow paths — a manifest/generator change.
