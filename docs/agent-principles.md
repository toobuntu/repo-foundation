...
- **Do not reflexively rewrite user-facing text because of a lint finding.** When a linter objects to the *wording* of something a person will read — a recovery command quoted in a refusal message, a specific hint, a documented example — stop and decide which to change, the text or the rule's verdict. Sometimes the finding is right and the wording was incidental; sometimes suppressing that one finding is right. The judgment is about clarity and context, and the one direction that is always wrong is trading a concrete copy-pasteable hint for silence. Where a suppression is the answer, scope it: in shell that is `# shellcheck disable=SCnnnn` immediately above the line, with the reason on the line above that — and the reason must not begin with a lowercase `# shellcheck`, which the directive parser claims (SC1072/SC1073). Where no per-finding suppression exists, accepting a standing warning is a legitimate outcome but never the preferred one — say so explicitly, and record what would retire it.
...
- **Conventional Commits, except in the Homebrew-aligned repositories.** The org standard is a `type(scope): subject` line — `feat`, `fix`, `docs`, `chore`, `refactor`, `test`, `build`, `ci` — which keeps the 50-char budget and makes the change's category readable without opening the diff. The exception is not a style preference and is not negotiable locally: Homebrew **prohibits** Conventional Commits in its taps and external commands, and the org syncs Homebrew's own `reject-conventional-commits.yml` into homebrew-babble and homebrew-cask-tools to enforce it there. Those two repositories follow Homebrew's commit style instead. Do not carry either style across the boundary, and check which side of it you are on before writing the subject.
...
- `--no-gpg-sign` disables signing. Do NOT add `-c commit.gpgsign=false`: it is redundant with the flag. (Historical note: a `sandbox.excludedCommands` entry matching that string was associated with commit hangs — reproduced within one session on 2026-07-21, mechanism never isolated — and was removed from the synced baseline on 2026-07-24; the redundancy alone is reason enough to keep the flag off.)
...
- **Tested 2026-07-21 (one session, with a control):** the two `sandbox.excludedCommands` entries behaved differently. `zizmor *` **works** — a standalone `zizmor --quiet .` runs unsandboxed, no panic, in seconds. zizmor runs offline unless a token is passed explicitly (verified on 1.27: `GH_TOKEN=$(gh auth token) zizmor .` for an online audit; without `GH_TOKEN` it stays offline even with the gh CLI logged in — corrected 2026-07-22 from an earlier auto-detection claim), which is the right default for a lint gate anyway. An env-prefixed commit matching the `git -c commit.gpgsign=false commit *` entry stalled before git executed, three times in that session, while the identical command without `-c commit.gpgsign=false` committed in seconds — but the same commands had also worked in earlier sessions, so the mechanism was never isolated and the hang is an association, not a proven cause. Disposition applied 2026-07-24: the git entry is removed from the baseline (a commit gate should not depend on the network, and the `-c` form is redundant with `--no-gpg-sign` regardless); the zizmor entry stays for ad-hoc online audits.
- **The commit recipe:** run `git commit --no-gpg-sign …` **without** `-c commit.gpgsign=false` (the `--no-gpg-sign` flag alone disables signing), and prepend `ZIZMOR_OFFLINE=true` (value `true`/`false`, not `1`) — the hook's zizmor runs offline and sandboxed (only online audits are suppressed; they need network and a `--gh-token` anyway), so the **full** hook still runs, no `--no-verify` or sandbox widening. Don't set it globally in `env`.
...
- **Start of session**: run `scripts/ai/ai-session.sh start` (snapshots `.ai/progress.md` for the end-of-session comparison; a no-op where there is no `.ai/` layer), then read `.ai/org/memory.md` (org-wide knowledge, synced from repo-foundation), `.ai/memory.md` (this repo's knowledge), and `.ai/progress.md` (the previous session's state, if present — it is gitignored and per-developer; seed it from the committed `.ai/progress.template.md` when absent).
- **End of session**: append durable learnings to `.ai/memory.md` (dated entries, `## YYYY-MM-DD — Topic`, append-only — a correction is a new entry naming what it supersedes), rewrite `.ai/progress.md` to the current state and next action, then run `scripts/ai/ai-session.sh end` and **account in the handoff report for every line it lists as removed**. Most removals are correct — finished status should go away. The one that is not is a line recording a COMMITMENT rather than a status, which had to graduate somewhere durable (a dispatch row, an issue, an ADR, `.ai/memory.md`) before it left the file. Stating each removal is what makes that case visible; a rewrite with no diff hides it completely, which is how a queued first-sync cleanup item was silently lost on 2026-07-25.
