...
- **A lint finding never rewrites user-facing text.** When a linter objects to the *wording* of something a person will read — a recovery command quoted in a refusal message, a specific hint, a documented example — keep the clearer text and take a scoped, commented disable. Reword only where the wording was incidental to begin with. The message is the deliverable and a clean lint run is bookkeeping, so trading a concrete copy-pasteable hint for silence is the wrong direction. In shell that means `# shellcheck disable=SCnnnn` immediately above the line, with the reason on the line above that — and the reason must not begin with a lowercase `# shellcheck`, which the directive parser claims (SC1072/SC1073).
