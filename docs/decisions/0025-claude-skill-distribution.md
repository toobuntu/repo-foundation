...
- **The bucket-scope copies are removed.** Each duplicates toobuntu/blackoutd's own, so nothing is lost by deleting them there, and three mis-fire everywhere else. A maintainer step: that directory sits outside every repository tree and outside the agent sandbox.
