...
Four more skills sat at the *bucket* scope: the `.claude/skills/` of the directory that holds every toobuntu clone on a maintainer's machine. All four (`dev-cycle`, `release-check`, `test`, `review-pr`) duplicate toobuntu/blackoutd's own copies, and three name toobuntu/blackoutd explicitly ("run toobuntu/blackoutd's edit/build/reload cycle", "toobuntu/blackoutd's pre-commit hook"). A skill at that scope loads in *every* repository under the directory, so an agent working in toobuntu/homebrew-cask-tools is offered toobuntu/blackoutd's build cycle.
...
- **What does not.** A skill naming one repository's toolchain stays in that repository, unsynced: toobuntu/blackoutd's `dev-cycle`, `release-check` and `test`; toobuntu/bob-book's `toobuntu/bob-book-transcription`.
- **The bucket-scope copies stay.** (Amended 2026-08-04, reversing this ADR at acceptance, which had said to remove them.) The original reasoning treated the bucket as a place skills leak *from* and never asked what it was *for*. It is deliberate: rooting a session at that directory is what makes every repository under it writable, because the agent sandbox's writable area is the project tree. Measured that day in a bucket-rooted session — the skill list showed exactly the four bucket copies, all scoped `project`, and **no** path-prefixed subdirectory skills such as `blackoutd:dev-cycle`. Subdirectory skills do not load. So those four are that session's entire working set, not redundancy, and deleting them would have left it with none.
- **`review-pr` is superseded rather than promoted.** Its content is "fetch a pull request's review state, including the inline comments `gh pr view` omits" — which is step 0 of `tb-review-triage`, carrying the same warning about the separate REST endpoint. Consumers get the behavior through the synced skill; toobuntu/blackoutd may retire its local copy.
...
- Bad, and open: a bucket-rooted session gets **none** of the `tb-*` skills. It sees only what sits in the bucket's own `.claude/skills/`, so `tb-session-close` and `tb-review-triage` are unavailable in exactly the sessions that span several repositories. Two constraints shape any fix. The bucket is **not a git repository**, so the sync engine — which writes through the GitHub API to consumer repositories — cannot reach it; delivery there has to be a local copy, by hand or by a `foundation-init`-style script. And its skills would want namespacing, since a flat bucket set offers one repository's `dev-cycle` while working in another. Recorded rather than settled: the distribution question for that scope is open.
