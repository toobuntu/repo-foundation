...
- **The bucket-scope copies are removed.** Each duplicates a copy toobuntu/blackoutd already carries, so nothing is lost by deleting them there, and three misfire everywhere else. A maintainer step: that directory sits outside every repository tree and outside the agent sandbox.
