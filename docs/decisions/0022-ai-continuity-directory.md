...
The system is two-tier. **Org-wide** state lives in `workspace/.ai/`, where `progress.md` *is* committed because it is shared coordination state, and sessions dispatch from `workspace/dispatch.md` as the single front door. **Per-repo** state lives in each repo's `.ai/`. A session rooted in a sandbox clone that cannot write `workspace/` relays through a gitignored `<repo>/.ai/org/progress-relay.md` drop that the maintainer, or the next desktop-rooted session, merges upward.
