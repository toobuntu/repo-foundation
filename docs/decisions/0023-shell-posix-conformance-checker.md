...
Second amendment, same day: the severity drop exposed a **tool-version skew** the previous threshold had hidden. `shell-lint.yml` used the Ubuntu runner's pre-installed shellcheck (0.9.x) while developers run Homebrew's (0.11.0), and the two disagree at the lower tiers — 0.9.x reports SC2015 on the conjunction guard `[ -n "$a" ] && [ -n "$b" ] || fallback`, which 0.11.0 correctly leaves alone. CI therefore failed on code the pre-commit hook had just passed, which is the worst direction for the skew to run: it moves discovery past the commit, and the hook stops being trustworthy. The workflow now installs shellcheck from Homebrew alongside shfmt, as that file already did for shfmt and for the same stated reason, and echoes both versions into the log. The general rule this illustrates: a gate shared between a local hook and CI must pin, or otherwise align, the tool that decides its verdict — the lower the severity threshold, the more of the tool's surface that decision depends on.
