...
Two measured negatives worth recording, because both are the obvious next idea:

- **`file --mime-encoding` cannot replace `--mime-type`.** It is tempting — it answers `binary` or `us-ascii`/`utf-8` directly, needs no denylist, and emits one line per file even for a universal binary. But it reports `binary` for a shell script carrying an embedded NUL, which is exactly the file that must be reported. It classifies *bytes*; the policy question is whether a human reviews the file as source, which is what `--mime-type` answers (that same script is `text/x-shellscript`). Adopting it would silently reopen the hole the encoding check closed.
- **Batching `--mime-type` is safe after all, keyed on `--separator`.** The earlier claim that a batch would desync at the first universal binary was wrong in one respect: `file(1)` emits exactly one separator-bearing line per *input* file and that line **carries the path**, so the mapping never depends on output position; a fat binary's extra per-architecture lines use a literal tab and are ignored. The audit path therefore batches (measured 2.1s versus roughly 25s per-file over ~1000 files, chunked below `ARG_MAX`), while the gate path stays lazy — it classifies only files that already produced a finding, so a clean tree still spawns nothing.
