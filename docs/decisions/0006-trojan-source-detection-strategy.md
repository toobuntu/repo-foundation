...
The tree scope also moves out from under the test suite: as a step in `spec.yml` it silently never ran when RSpec failed, and a security gate downstream of test health is the wrong dependency direction. The dedicated `unicode-audit` workflow runs scope 3 on its own — currently as a **two-OS matrix trial**, because the binary/text confirmation shells out to `file(1)` and the MIME denylist was censused against BSD `file`, not the GNU implementation. The denylist direction makes a vocabulary gap fail toward noise rather than a silent skip, so the cheap runner is the candidate: macOS gates, ubuntu runs `continue-on-error`, both legs emit a sorted per-file classification report (`--classify-report`), and a compare job diffs the decision column into the step summary. Sunset, decided now: after ~20 runs or a month of decision-column agreement, drop the macOS leg and the `continue-on-error`, promote the workflow into the `ci_core` sync set with a per-ecosystem dependency step, and remove the interim `spec.yml` step in the same change. Until then the workflow is repo-foundation-only and the `spec.yml` step is what consumers run.
