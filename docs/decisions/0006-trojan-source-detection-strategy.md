...
> **The two-artifact split above is superseded** by the 2026-07-29 amendment, "one detector; the plugin delegates". The `80-unicode` plugin now runs `scripts/lint-unicode.sh --scope=staged`, so the commit-time gate performs the full Cf/Cc sweep wherever python3 exists and falls back to the same sixteen-codepoint grep set otherwise. This paragraph is left as the decision that was taken; read it with that amendment.
