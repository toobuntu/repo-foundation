...
**Suppression requires BOTH of `file(1)`'s answers**, which are two independent axes:

- `--mime-encoding` describes the **bytes**: `binary`, or a character encoding. It cannot decide alone — it reports `binary` for a shell script carrying an embedded NUL, exactly the file that must be reported, so using it as the sole test would silently reopen the hole the encoding check closed.
...
So a finding is suppressed only where the bytes are binary **and** every reported format is a known-binary one. Either query failing — `file(1)` absent, erroring, or unparseable — yields a report. The conjunction narrows suppression, which is the direction a security gate should fail, and is the same reasoning as preferring a denylist to an allowlist.

One measured correction worth recording, since the earlier design turned on it: **batching `--mime-type` is safe, keyed on `--separator`.** The claim that a batch would desync at the first universal binary was wrong about the mechanism — `file(1)` emits exactly one separator-bearing line per *input* file and that line **carries the path**, so the mapping never depends on output position; a fat binary's extra per-architecture lines use a literal tab and are ignored. Both queries therefore run as batched passes (measured ~2.6s together, versus roughly 25s per-file over ~1000 files, chunked below `ARG_MAX`), while the gate path stays lazy — it classifies only files that already produced a finding, so a clean tree still spawns nothing. A two-step arrangement (batch the cheap encoding pass, then call `--mime-type` only on the binaries) reaches the same decisions and was the right shape while `--mime-type` was believed unbatchable; with the separator it is one extra pass over a subset rather than a saving, so the flat conjunction is simpler for the same result.
