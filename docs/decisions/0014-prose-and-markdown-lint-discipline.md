...
- **One style, synced.** The `Toobuntu` style lives in repo-foundation and syncs to consumers (ADR 0003), behind a single `accept.txt` vocabulary. It absorbs the former `Didan.*` rules (American-spelling, impersonal-voice, abbreviation-plurals) plus adopted rules from the Homebrew, GitLab, and Elastic style sets. A consumer consumes the synced style instead of maintaining its own, so no consumer ever receives two styles with conflicting swaps.
