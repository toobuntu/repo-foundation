...
- **Distributed via the `prose_plugin` set** to the hook-carrying `prose_lint` consumers (blackoutd, zman-didan, homebrew-babble, bob-book, cert-automation, homebrew-cask-tools). dot-github takes no hooks and no prose lint, so it is not mapped.
