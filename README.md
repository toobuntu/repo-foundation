...
- **Sync to consumers.** `.github/workflows/sync-to-consumers.yml` runs the engine for each consumer in `sync-manifest.yaml` and opens a sync pull request (ephemeral `sync/<run-id>` branch, Verified per-file commits) when any managed output changes — a canonical file, a generated file, or a managed region. A quiet scheduled run means the consumers are converged.
