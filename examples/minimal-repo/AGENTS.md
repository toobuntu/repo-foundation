...
A worked example of a repo-foundation consumer's `AGENTS.md`. `CLAUDE.md` is a
one-line `@AGENTS.md` pointer. The block between the sentinels below is managed
by the sync; this repo's own context lives outside it.
...
This section is outside the managed region, so the sync never touches it. A real
consumer fills it with its own summary, build and test commands, safety
invariants, and project-specific tools — for example:
